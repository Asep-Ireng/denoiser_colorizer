"""
Training Script for Plain/Standard U-Net (Baseline)

Format is identical to train_residual.py for fair comparison.
Changes:
1. Imports PlainUNet instead of ResUNet
2. Saves weights to 'weights/plain'
"""
...
def train_epoch(model, dataloader, optimizer):
    ...
    pbar = tqdm(dataloader, desc="Training (Plain)", unit="batch")
...
def main():
    ...
    # --- PENTING: BEDA FOLDER OUTPUT ---
    parser.add_argument('--output_dir', type=str, default='weights/plain', 
                        help='Save weights to a different folder to avoid overwriting')
    ...
    print("\nInitialize PLAIN Model (Input Channel: 1)...")
    # --- MENGGUNAKAN PLAIN UNET ---
    model = PlainUNet(in_nc=1, out_nc=1).to(DEVICE)
...
# --- 1. SIMPAN MODEL TIAP EPOCH (Checkpoint) ---
        ...
        epoch_save_path = os.path.join(args.output_dir, f'plain_denoiser_epochss{epoch}.pth')
        ...
        if v_psnr > best_psnr:
            ...
            best_save_path = os.path.join(args.output_dir, 'plain_denoiser_best20.pth')
