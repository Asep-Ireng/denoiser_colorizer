...
def load_models():
    ...
    # FPN Arch-only
    print("Loading FPN Arch...")
    fpn_arch = UNetResFPN(in_nc=2, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R')
    fpn_arch_weights = PROJECT_ROOT / 'weights' / 'fpn' / 'fpn_arch_only.pth'
    if fpn_arch_weights.exists():
        fpn_arch.load_state_dict(torch.load(fpn_arch_weights, map_location=DEVICE, weights_only=True))
    fpn_arch.to(DEVICE).eval()
    models['FPN_Arch'] = fpn_arch
    
    # FPN Trained
    print("Loading FPN Trained...")
    fpn_trained = UNetResFPN(in_nc=2, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R')
    fpn_trained_weights = PROJECT_ROOT / 'weights' / 'fpn' / 'fpn_best.pth'
    if fpn_trained_weights.exists():
        fpn_trained.load_state_dict(torch.load(fpn_trained_weights, map_location=DEVICE, weights_only=True))
        fpn_trained.to(DEVICE).eval()
        models['FPN_Trained'] = fpn_trained
    else:
        print("  (FPN Trained not found)")
